...
type callCfg struct {
	failed    string //上次请求失败的节点
	failedMsg string //上次请求失败的节点
	retry     bool
	logId     string
	req       *Req
}
...
func withCallFailed(failed string) callOpt {
	return callOpt(func(opt *callCfg) {
		opt.failed = failed
	})
}
func withCallFailedMsg(failedMsg error) callOpt {
	return callOpt(func(opt *callCfg) {
		if failedMsg != nil {
			opt.failedMsg = failedMsg.Error()
		}
	})
}
...
func (c *Caller) createCall(apiVersion, service string, r *Req, tm time.Duration) (s *Res, errcode int32, e error) {
	...
	tmpFlag, prx, conn, e := c.getPrxy(apiVersion, service, span, withCallLogId(logId), withCallReq(r))
	...
	// 进入重试,换机器
	if e != nil && c.retry > 0 {
		return c.retryCall(apiVersion, service, r, tm, span, withCallReq(r), withCallRetry(true), withCallFailed(conn.Addr), withCallFailedMsg(e))
	}
...
func (c *Caller) oneShortCall(apiVersion, service string, r *Req, tm time.Duration) (s *Res, errcode int32, e error) {
	...
	tmpFlag, prx, conn, e := c.getPrxy(apiVersion, service, spnxt, withCallLogId(logId), withCallReq(r))
	if e != nil {
	...
	c.cli.Log.Infow("record doCall perf",
		"fn", "oneShort:Call", "dur", dur, "tm", tm.String(), "errcode", errcode, "e", e)
	c.cli.updateLb(service, conn.Addr, s, errcode, e, dur)
	if e != nil {
		c.cli.Log.Errorw("oneShortCall:doCall", "logId", logId, "error", e.Error(), "service", service, "addr", conn.Addr)
	...
	// 进入重试,换机器
	if e != nil && c.retry > 0 {
		return c.retryCall(apiVersion, service, r, tm, span, withCallLogId(logId), withCallRetry(true), withCallFailed(conn.Addr), withCallFailedMsg(e))
	}
...
func (c *Caller) getPrxy(apiVersion, service string, sp *Span, opt ...callOpt) (bool, utils.XsfCallClient, *SFConn, error) {
	callCfgInst := &callCfg{}
	...
	if c.cli.lb.isHash() {
		...
		if !hashKeyOk {
			...
			c.cli.Log.Infow("set local ip+port for default", "ipStr", ipStr, "portStr", portStr, "logId", callCfgInst.logId)
	...
	if len(c.lbname) != 0 { //启用了远端LB模式
		...
		lbp.WithFailed(callCfgInst.failed)
	...
	if e != nil {
		c.cli.Log.Infow("c.cli.GetConn(lbp) failed", "logId", callCfgInst.logId)
...
func (c *Caller) GetSrv(service string) (addr []string, addrErr error) {
	...
	*/
	for i := 0; i < 10; i++ {
		...
		if addrErr == nil && addrTmp[0] == startIx[0] {
			break
		}
		if addrErr != nil || len(addrTmp) > 0 {
...
func (c *Caller) stringAdd() {
	...
	for k, v := range inTmp {
		vt := uint8(v)
		if (vt >= 48 && vt < 57) || (vt >= 65 && vt < 90) || (vt >= 97 && vt < 122) {
			inTmp[k] = v + 1
			cf = true
			break
		}
...
//func (c *Caller) filterConnAddr(conn *SFConn) string {
//	if utils.IsNil(conn) {
//		return ""
//	}
//	return conn.Addr
...
// retryCall
func (c *Caller) retryCall(apiVersion, service string, r *Req, tm time.Duration, sp *Span, opt ...callOpt) (s *Res, errcode int32, e error) {
	...
	var failed string   //保留重试过程中失败的节点
	var failedMsg error //保留重试过程中失败的节点的具体错误
	for i := 0; i < c.retry; i++ {
		...
		spnxt = spnxt.WithRpcCallType()
		var tmpFlag bool
		if len(callCfgInst.failed) != 0 || len(failed) != 0 {
			//说明重试也错了，能怎么办呢，改吧
			report := callCfgInst.failed
			if len(failed) != 0 {
				report = failed
			}
			tmpFlag, prx, conn, e = c.getPrxy(apiVersion, service, sp, append(opt, withCallFailed(report), withCallFailedMsg(failedMsg))...)
		} else {
			tmpFlag, prx, conn, e = c.getPrxy(apiVersion, service, sp, opt...)
		}
		if e != nil {
			if conn != nil {
				spnxt.WithTag("addr", conn.Addr)
		...
		callBase := time.Now()
		s, errcode, e = c.doCall(prx, r, tm)
		c.cli.Log.Infow("record doCall perf", "fn", "retryCall", "dur", time.Since(callBase).Nanoseconds())

		if e != nil {
			c.cli.Log.Errorw("retryCall", "logId", callCfgInst.logId, "error", e.Error(), "service", service, "addr:", conn.Addr)
