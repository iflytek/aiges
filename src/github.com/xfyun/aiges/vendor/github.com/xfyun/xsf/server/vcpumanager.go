...
func getVCpuCfg(cfgVer, cfgPrj, cfgGroup, cfgService, cfgName, cfgUrl string, cfgMode utils.CfgMode) (vCpuMap map[string]interface{}, err error) {
...
func newVCpuWindow(timePerSlice time.Duration, winSize int64) *vCpuWindow {
	vCpuWindowInst := vCpuWindow{}
	vCpuWindowInst.Init(timePerSlice, winSize)
	return &vCpuWindowInst
}

//清理闲时的节点数据
func (s *vCpuWindow) reset() {
...
func (s *vCpuWindow) Init(timePerSlice time.Duration, winSize int64) {
...
//入参为tps的s
func (s *vCpuWindow) getStats() []float64 {
	var index = s.locationIndex()
...
func (s *vCpuWindow) clearBetween(fromIndex, toIndex int64) {
...
type VCpuManager struct {
	...
	//window *vCpuWindow //数据窗口,2019-08-01 15:43:12 移除数据窗口
