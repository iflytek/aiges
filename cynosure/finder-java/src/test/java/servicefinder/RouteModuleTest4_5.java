...
/**
 * 5）从不在路由规则，调整到路由规则。
 * 预期：之前非路由规则的实例下线，路由规则中的实例上线
 */
public class RouteModuleTest4_5 {
    ...
    static String  routeData = "[{\"id\":\"1\",\"consumer\":[\"c2\",\"c3\"],\"provider\":[\"p1\",\"p2\",\"p3\"],\"only\":\"Y\"}]";

    String changedRouteDataAdd = "[{\"id\":\"1\",\"consumer\":[\"c1\",\"c3\",\"c2\"],\"provider\":[\"p1\",\"p2\",\"p3\"],\"only\":\"Y\"}]";
    ...
    private void testSubscribeService() throws Exception {
        ...
        CommonResult<List<Service>> listCommonResult = finderManager.useAndSubscribeService(ListUtil.collectAsList(value), new ServiceHandle() {
            ...
            public boolean onServiceInstanceChanged(String serviceName, List<InstanceChangedEvent> eventList) {
                ...
                RouteModuleTest4_5.this.eventList=eventList;
        ...
        List added = ListUtil.collectAsArrayList( "p1","p2","p3");
        List removed = ListUtil.collectAsArrayList( "p4","p5");
    ...
    private void changeRouteFromNtoY(){
        ...
        zkHelper.addOrUpdatePersistentNode(baseConfigPath+"/route",ByteUtil.getZkBytes(changedRouteDataAdd.getBytes(),"1234567890"));
    }
}
