        ...
        if (traceback_obj != NULL) {
            ...
            if (pTraceModule != NULL) {
                ...
                if (pModuleDict != NULL) {
                    ...
                    if (pFunc != NULL) {
                        PyObject *errList = PyObject_CallFunctionObjArgs(pFunc, type_obj, value_obj, traceback_obj,
                                                                         NULL);
                        if (errList != NULL) {
...
int pyDictGetData(PyObject *obj, void **buffData, std::string sid, int &dataSize) {
    std::string errRlt = "";
    int ret = 0;
    PyObject *data = PyDict_GetItemString(obj, DATA_DATA.c_str());
    if (data == NULL) {
        ret = WRAPPER::CError::RltDataDataInvalid;
        errRlt = log_python_exception();
        if (errRlt != "") {
            spdlog::error("Get Data  error:{}, sid:{}", errRlt, sid);
        }
        return ret;
    }
    Py_ssize_t actual_size;
    PyObject *len = PyDict_GetItemString(obj, DATA_LEN.c_str());;
    if (len == NULL) {
        ret = WRAPPER::CError::RltDataLenInvalid;
        errRlt = log_python_exception();
        if (errRlt != "") {
            spdlog::error("Get Data  error:{}, sid:{}", errRlt, sid);
        }
    ...
    int userDefineDataSize = 0;
    char *st;
    if (PyLong_Check(len)) {
        dataSize = (int) PyLong_AsLong(len);
    }
    //    PyArg_Parse(len, "i", &userDefineDataSize);
    // 数据处理
    if (PyBytes_Check(data)) {
        ...
        dataSize = (int) actual_size;
        memcpy(r, st, dataSize);
        *buffData = r;

    } else if PyUnicode_Check(data) {
        PyObject *tmp = PyUnicode_AsUTF8String(data);
        st = strdup(PyBytes_AsString(tmp));
        *buffData = strdup(st);
        Py_XDECREF(tmp);
    } else {
        ret = WRAPPER::CError::RltDataDataInvalid;
        spdlog::error("No bytes Data and Unicode Data,  error:{}, sid:{}", errRlt, sid);
        return ret;
    }
    return 0;
...
char *pyDictStrToChar(PyObject *obj, std::string itemKey, std::string sid, int &ret) {
    ...
    if (PyBytes_Check(pyValue)) {
        ...
        spdlog::error("not Support this DataType , key: {},,sid:{}", itemKey, sid);
        return NULL;
    }
