...
type LbAdapter struct {
	...
	subRouterTypes []string //子路由类型列表(如:["svc_gray","svc_hefei"])
...
//配置负载的子路由类型列表(如:["svc_gray","svc_hefei"])
...
func (l *LbAdapter) Login(svc string, totalInst, idleInst, bestInst int32, param map[string]string) error {
	...
	if LoginErr := l.lc.Login(l.svc, totalInst, idleInst, bestInst, param); nil != LoginErr {
