...
//			max, min, avg := slidingDelayWindowsItem.getStats()
//			registryInst.WithLabelValues(
...
func healthCheck(target string) bool {
	conn, connErr := grpc.Dial(
		target,
		grpc.WithInsecure(),
		grpc.WithTimeout(time.Millisecond*500),
		grpc.WithBlock())
	if nil != connErr {
	...
	cmdServerResp, cmdServerRespErr := c.Cmdserver(
		context.Background(),
		&utils.Request{Query: string(query),
			Headers: string(header),
			Body:    "",
		},
	)
	if nil != cmdServerRespErr {
		return false
	}
