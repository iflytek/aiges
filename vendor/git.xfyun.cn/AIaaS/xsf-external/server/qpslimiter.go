...
func (l *QpsLimiter) SetQpsLimiter(opts ...QpsLimiterOpt) error {
	...
	go l.guarder()
	return l.reporter.Login(
		l.bc.CfgData.Service,
		l.maxReqCount,
		l.maxReqCount-l.currentReqCount,
		l.bestReqCount,
		map[string]string{
			"bc": func() string { res, _ := json.Marshal(l.bc); return string(res) }(),
		},
	)
}
...
func (l *QpsLimiter) Update(maxReq, idleInst, bestReq int32) error {
	l.logger.Debugf("QpsLimiter -> l.reporter.Update(maxReq:%v, idleInst:%v, bestReq:%v)",
		maxReq, idleInst, bestReq)
