...
func newLB(log *utils.Logger, r uint, m LBMode, o ...connOpt) *loadBalance {
	...
	if func(obj interface{}) bool {
		...
		return nil == (*eface)(unsafe.Pointer(&obj)).data
	}(lb.lbi) {
		return nil
	}
...
func (lb *loadBalance) findWithID(lbp *LBParams, id string) (tmp bool, conn *SFConn, e error) {
	...
	if len(addr) > 0 {
		...
		//2019-05-27 16:38:50
		if !lbp.retry && lb.isLocalPriority() && "" != lbp.localIp && 0 != len(allAddr) {
			tmp, conn, e = lb.localPriority(lbp, allAddr, id)
			if nil == e {
				return tmp, conn, e
			}
		} else {
		...
		/////////////////////////
		//2018/10/11 11:36
		tmp, conn, e = func() (tmp bool, conn *SFConn, e error) {
			return lb.getConns(id, addr, lbp)
		}()
		if nil == e {
			return tmp, conn, e
		}
...
func (lb *loadBalance) getConns(id string, addr []string, lbp *LBParams) (tmp bool, conn *SFConn, e error) {
	addrIx := 0
	var i uint
	for i = 0; i < lb.r; i++ {
		lb.log.Infof("loadBalance.findWithID id: %s, addr:%s,lb.r:%v",
			id, addr[addrIx%len(addr)], lb.r)
		...
		if nil == e {
			...
			lb.log.Warnw("lb.conns.Get failed",
				"addr", addr[addrIx%len(addr)], "logId", lbp.logId, "err", e.Error())
		}
		addrIx++
	}
	return
...
func (lb *loadBalance) localPriority(lbp *LBParams, allAddr []string, id string) (tmp bool, conn *SFConn, e error) {
	cnt := len(allAddr)
	lb.log.Infow(
		"loadBalance.FindWithID,about to use local address first",
		"logId", lbp.logId, "name", lbp.name, "nbest", lbp.nbest, "localIp", lbp.localIp, "allAddrCnt", cnt, "retry", lbp.retry)
	...
	for _, val := range allAddr {
		...
		if strings.Contains(val, lbp.localIp) {
			...
			if e == nil {
				...
				lb.log.Warnw("lb.conns.Get failed",
					"addr", val, "logId", lbp.logId, "err", e.Error())
			}
		} else {
			if cnt <= 0 {
				e = fmt.Errorf("ignore local priority")
			}
		}
	}
	return
...
func (lb *loadBalance) findWithHandle(handle string) (*SFConn, error) {
	...
	// 根据handle解析出ip、port
	addr, addrErr := lb.parseHandle(handle)
	if nil != addrErr {
		return nil, addrErr
	}
	// 根据地址获取可用连接
	return func() (*SFConn, error) {
		return lb.getConnWithAddr(handle, addr)
	}()
}

func (lb *loadBalance) getConnWithAddr(handle string, addr string) (*SFConn, error) {
	var i uint
	for i = 0; i < lb.r; i++ {
...
func (lb *loadBalance) parseHandle(handle string) (string, error) {
...
func (lb *loadBalance) handleToIP(h string) (string, error) {
	...
	return strconv.Itoa(int(ip1)) +
		"." +
		strconv.Itoa(int(ip2)) +
		"." +
		strconv.Itoa(int(ip3)) +
		"." +
		strconv.Itoa(int(ip4)), nil
}
