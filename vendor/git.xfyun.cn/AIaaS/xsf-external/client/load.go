...
func newItem(valueIn string, priorityIn int) *Item {
	return &Item{
		value: func() atomic.Value {
			var value atomic.Value
			value.Store(valueIn)
			return value
		}(),
		priority: int64(priorityIn),
	}
}
...
type cellCalculator struct {
	healthData  *healthWin
	latencyData *latencyWin
	vCpu        int64
}

func newCellCalculator(timePerSlice time.Duration, winSize int64) *cellCalculator {
	healthWindowInst := newHealthWin(timePerSlice, winSize)
	latencyWindowInst := newLatencyWin(timePerSlice, winSize)
