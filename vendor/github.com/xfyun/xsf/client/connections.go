...
func (cm *connPoolMeta) allocGrpcConn(done chan error, addr string, ms time.Duration) bool {
...
func (cm *connPoolMeta) fillConnPool(done chan error, addr string, ms time.Duration) (filler bool) {
		...
		go func(done chan error) {
			...
			for {
				if cm.size < cm.max {
					//本次建连是否成功
					//失败则退出
					if !cm.allocGrpcConn(done, addr, ms) {
						close(done)
						break
					}
				} else {
...
func (cm *connPoolMeta) newconnPool(addr string, ms time.Duration) (*grpc.ClientConn, error) {
	...
	var filler bool
	if cm.size < cm.max {
		filler = cm.fillConnPool(done, addr, ms)
	}
