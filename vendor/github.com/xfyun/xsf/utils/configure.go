...
func (c *Configure) OnConfigFilesAdded(configs map[string]*finder.Config) bool {
	return true
}

func (c *Configure) OnConfigFilesRemoved(configNames []string) bool {
	return true
}
...
func (c *Configure) GenNewCfg(co *CfgOption) (*Configure, error) {
	...
	if co.prj != c.opt.prj || co.group != c.opt.group || co.ver != c.opt.ver || co.mode != c.opt.mode {
