...
type SessionManagerOptOTF func(*SessionManager)

func WithSessionManagerAuthFilterWin(win time.Duration) SessionManagerOpt {
	return func(sm *SessionManager) {
		sm.authFilterWin = win
	}
}
...
func WithSessionManagerMaxLicOTF(maxLic int32) SessionManagerOptOTF {
	return func(sm *SessionManager) {
		sm.expectMaxLic = maxLic
	}
}
...
func rstFormatter(rst []callRstItem) interface{} {
	...
	for ix, rstItem := range rst {
		...
		r = append(r, fmt.Sprintf("->%v:addr:%v failed,s:%v,errcode:%v,e:%v", ix, rstItem.addr, func(in *Res) interface{} {
			if in == nil {
				return nil
			}
...
func (s *SessionManager) UpdateOTF(ctx context.Context, opts ...SessionManagerOptOTF) error {
	defer func() {
		loggerStd.Printf("fn:UpdateOTF,action-exiting\n")
	}()
	for _, opt := range opts {
		opt(s)
	}

	//如果授权是扩张，直接修改maxLic即可
	if atomic.LoadInt32(&s.expectMaxLic) > atomic.LoadInt32(&s.MaxLic) {
		loggerStd.Printf("fn:UpdateOTF,action-expand:set %v to maxLic(current:%v)\n",
			atomic.LoadInt32(&s.expectMaxLic), atomic.LoadInt32(&s.MaxLic))
		atomic.StoreInt32(&s.MaxLic, s.expectMaxLic)
	}
	...
	if atomic.LoadInt32(&s.expectMaxLic) < atomic.LoadInt32(&s.MaxLic) {
		for {
			...
			//如果正在使用授权比期望授权低，直接修改maxLic即可，否则持续轮训或超时退出
			if atomic.LoadInt32(&s.NowLic) < atomic.LoadInt32(&s.expectMaxLic) {
				loggerStd.Printf("fn:UpdateOTF,action-shrink:set %v to maxLic(current:%v)\n",
					atomic.LoadInt32(&s.expectMaxLic), atomic.LoadInt32(&s.MaxLic))
				atomic.StoreInt32(&s.MaxLic, s.expectMaxLic)
				return nil
			}
...
func (s *SessionManager) Init(opts ...SessionManagerOpt) (callback func() error, err error) {
	...
	s.authFilter = (&authFilter{}).init(s.authFilterWin)
	...
	callback = func() error {
		...
		/*
			老版本lb的上报
		*/
		if s.reporter.able != 0 {
			loginErr := s.reporter.Login(s.bc.CfgData.Service, s.MaxLic, atomic.LoadInt32(&s.MaxLic)-atomic.LoadInt32(&s.NowLic), s.BestLic, map[string]string{"bc": func() string {
		...
		{
			{
				...
				s.logger.Debugf("SessionManager -> s.update(s.MaxLic:%v, idle:%v, s.BestLic:%v)", s.MaxLic, idle, s.BestLic)
				s.update(s.MaxLic, idle, s.BestLic)
			}
				...
				case <-ticker.C:
					{
						...
						if math.Abs(float64(NowLic-lastMilepost)) >= float64(s.wave) {
							...
							s.logger.Debugf("SessionManager -> lastMilepost:%v,s.update(s.MaxLic:%v, idleInst:%v, s.BestLic:%v)", lastMilepost, s.MaxLic, idleInst, s.BestLic)
							s.update(s.MaxLic, idleInst, s.BestLic)
							lastMilepost = NowLic
						}
					}
				}
...
func (s *SessionManager) getAuthInfo() (maxLic, idle, bestLic int32) {
	...
	maxLic = s.MaxLic
	idle = s.authFilter.filter(atomic.LoadInt32(&s.MaxLic) - atomic.LoadInt32(&s.NowLic))
	bestLic = s.BestLic
...
func (s *SessionManager) update(totalInst, idleInst, bestInst int32) error {
	s.logger.Debugf("SessionManager -> s.reporter.update(totalInst:%v, idleInst:%v, bestInst:%v)", totalInst, idleInst, bestInst)
...
func (s *SessionManager) GetSessionData(sessionTag interface{}) (interface{}, error) {
	...
	if !sessionDataOk {
		return nil, fmt.Errorf("sessionData -> %v,sessionDataOk ->%v,can't get the value of sessionTag %s from map", sessionData, sessionDataOk, sessionTag)
	}
	...
	if !sessionDataNodeOk {
		...
		sessionDataNode.DeadTime.Store(time.Now().Add(s.Timeout))
...
func (s *SessionManager) SetSessionData(sessionTag interface{}, svcData interface{}, callback func(sessionTag interface{}, svcData interface{}, Exception ...CallBackException)) error {
	if atomic.AddInt32(&s.NowLic, 1) > atomic.LoadInt32(&s.MaxLic) {
		atomic.AddInt32(&s.NowLic, -1)
		s.logger.Errorw("SetSessionData failed,lack of license",
			"NowLic", atomic.LoadInt32(&s.NowLic), "MaxLic", atomic.LoadInt32(&s.MaxLic))
		return fmt.Errorf("lack of license -> now:%v,max:%v", s.NowLic, s.MaxLic)
	}
	{
		//debug
		s.logger.Debugw("SetSessionData", "tag", sessionTag, "now", time.Now(), "deadTime", time.Now().Add(s.Timeout), "timeout", s.Timeout,
			"NowLic", atomic.LoadInt32(&s.NowLic), "MaxLic", atomic.LoadInt32(&s.MaxLic))
	}
	deadTime := atomic.Value{}
	deadTime.Store(time.Now().Add(s.Timeout))
	node := &Node{Callback: callback, Data: svcData, sessionTag: sessionTag, DeadTime: deadTime}
	...
	if sessionDataOk {
		atomic.AddInt32(&s.NowLic, -1)
		s.logger.Debugw("SetSessionData,tag is already stored", "tag", sessionTag, "now", time.Now(), "deadTime", time.Now().Add(s.Timeout), "timeout", s.Timeout,
			"NowLic", atomic.LoadInt32(&s.NowLic), "MaxLic", atomic.LoadInt32(&s.MaxLic))
...
func (s *SessionManager) DelSessionData(sessionTag interface{}, exception ...CallBackException) {
	...
	*/
	...
	if !elemOk {
		s.logger.Debugw("DelSessionData,can't take tag", "tag", sessionTag, "now", time.Now(), "deadTime", time.Now().Add(s.Timeout), "timeout", s.Timeout,
			"NowLic", atomic.LoadInt32(&s.NowLic), "MaxLic", atomic.LoadInt32(&s.MaxLic))
		s.mMu.Unlock()
		return
	}
...
func (s *SessionManager) delelement() {
	s.logger.Debugf("SessionManager -> about to call delelement()")
	// 队尾出
	for {
		select {
		case sessionNode := <-s.out:
			{
				...
				s.logger.Infow("begin call node.callback", "sessionTag", node.sessionTag, "fn", "delement")
				node.Callback(node.sessionTag, node.Data, CallBackSessionTimeOut)
				s.logger.Infow("end call node.callback", "sessionTag", node.sessionTag, "fn", "delement")
				atomic.AddInt32(&s.NowLic, -1)
			}
		}
	}
...
//超时维护函数，将超时session提交给delelement函数
func (s *SessionManager) dealtimeout() {
	s.logger.Infof("SessionManager rollTimeout:%v -> about to call dealtimeout()", s.rollTimeout)
		...
		case <-timer.C:
			{
				...
				for {
					...
					if ts.After(sessionDataNode.DeadTime.Load().(time.Time)) {
						s.logger.Warnf("dealtimeout ->timestamp:%v,sessionTag:%v,NowLic:%v,MaxLic:%v", ts, sessionDataNode.sessionTag, atomic.LoadInt32(&s.NowLic), atomic.LoadInt32(&s.MaxLic))
						s.l.Remove(element)
						s.lMu.Unlock()
						s.mMu.Lock()
						if _, ok := s.m[sessionDataNode.sessionTag]; !ok {
							s.logger.Warnf("can't take of tag:%v of sessionDataNode", sessionDataNode.sessionTag)
							s.mMu.Unlock()
