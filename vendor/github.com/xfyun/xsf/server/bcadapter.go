...
func bcCheck(bc BootConfig) (BootConfig, error, error) {
	...
	if bc.CfgMode == -1 {
		if *Mode == -1 {
			...
			ext = fmt.Errorf("warning ->: %v", "the cfgmode use the command params instead of preset.")
	...
	if bc.CfgData.CfgName == "" {
		if *Cfg == "" {
			...
			ext = fmt.Errorf("warning ->: %v", "the cfgname use the command params instead of preset.")
	...
	if bc.CfgData.Service == "" {
		if *Service == "" {
			...
			ext = fmt.Errorf("warning ->: %v", "the service use the command params instead of preset.")
	...
	if bc.CfgData.Project == "" {
		if *Project == "" {
			...
			ext = fmt.Errorf("warning ->: %v", "the project use the command params instead of preset.")
		}
	}
	if bc.CfgData.Group == "" {
		if *Group == "" {
			...
			ext = fmt.Errorf("warning ->: %v", "the group use the command params instead of preset.")
	...
	if bc.CfgData.CompanionUrl == "" {
		if *CompanionUrl == "" {
			...
			ext = fmt.Errorf("warning ->: %v", "the companionurl use the command params instead of preset.")
