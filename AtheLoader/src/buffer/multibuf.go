...
type MultiBuf struct {
	...
	disorder  map[string] /*DataId*/ dataStream
...
func (mb *MultiBuf) Init(time uint, size uint, base uint) {
	...
	mb.disorder = make(map[string]dataStream)
...
func (mb *MultiBuf) ReadMergeData() (output []DataMeta, errNum int, errInfo error) {
	mb.migrate()
	tmpData := make(map[string]DataMeta)
	// select for timeout wait, block read
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(mb.waitTimeMs)*time.Millisecond)
	defer cancel()
	select {
	case <-mb.signal:
		errNum = frame.AigesErrorSeqChanClosed
...
// else return minimal frameId of dataStream
func minimalFrame(stream dataStream) (frameId int) {
	frameId = -1
